...
def get_request(self, url, params):
  ...
  res = request.get(f'{self.base_url}{url}' , params=params, headers=headers)
  return res.json()

def post_request(self, url, data):
  headers = self.headers
  headers['Authorization'] = f'Bearer {self.access_token}'
  res = request.post(f'{self.base_url}{url}', headers=headers, data=json.dumps(data))
  print(res.json())
  return res.json()

if __name__ == '__main__':
  client_id = "JAINAMDUCK-DK2100731"
  client_secret = "wnx3rTymOdR0O9QZPEn9HgfaBcocgQHvoqc8lNQvV5vihjLtTBe4TmxgkFnPXWJg"
  redirect_url = "http://127.0.0.1:65010"
  
  websocket_url = ""
  base_url = "https://spark.jainam.in"
  ...
  while True:
    ...
    if(order_update):
      # print(order_update)
      new_order_status = print(order_update["login_id"], order_update["order_status"])
